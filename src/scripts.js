...
(function() {
  ...
  function addListItem(pokemon) {
    ...
    $button = $(
      '<button type="button" class="btn btn-outline-info btn-lg btn-block" data-toggle="modal" data-target="#modalContent"> ' +
        pokemon.name +
        '</button>'
    );
  ...
  function showDetails(item) {
    pokemonRepository.loadDetails(item).then(function() {
      ...
      $nameElement = $(
        '<h1 class="modal-title" id="ModalLabel">' + item.name + '</h1>'
      );
